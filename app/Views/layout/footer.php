<footer class="bg-hmti-dark text-white pt-12 pb-6 border-t-4 border-hmti-accent">
    <div class="container mx-auto px-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
            <div>
                <h3 class="text-2xl font-bold mb-4 flex items-center">
                    HMTI FTTK
                </h3>
                <p class="text-gray-300 text-sm leading-relaxed">
                    Himpunan Mahasiswa Teknik Informatika<br>
                    Fakultas Teknik dan Teknologi Kemaritiman<br>
                    Universitas Maritim Raja Ali Haji
                </p>
            </div>

            <div>
                <h4 class="text-lg font-bold text-hmti-accent mb-4">Menu</h4>
                <ul class="space-y-2 text-sm">
                    <li><a href="/profil" class="hover:text-hmti-accent transition">Struktur Organisasi</a></li>
                    <li><a href="/berita" class="hover:text-hmti-accent transition">Berita & Kegiatan</a></li>
                    <li><a href="/proker" class="hover:text-hmti-accent transition">Program Kerja</a></li>
                    <li><a href="/mimbar" class="hover:text-hmti-accent transition">Mimbar Aspirasi</a></li>
                    <li><a href="/kontak" class="hover:text-hmti-accent transition">Kontak</a></li>
                </ul>
            </div>

            <div>
                <h4 class="text-lg font-bold text-hmti-accent mb-4">Hubungi Kami</h4>
                <p class="text-gray-300 text-sm mb-2">Gedung FTTK, Ruang HMTI, Senggarang</p>
                <p class="text-gray-300 text-sm">Email: hmti@umrah.ac.id</p>
            </div>
        </div>
        
        <div class="border-t border-gray-600 pt-6 text-center text-xs text-gray-400">
            &copy; <?= date('Y'); ?> HMTI UMRAH. Developed with <span class="text-red-500">❤</span> by JebatBois.
        </div>
    </div>
</footer>